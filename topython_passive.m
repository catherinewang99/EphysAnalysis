...
% path = 'J:\ephys_data\CW478\';

mkdir([path 'python'])

lst = dir(path);
for j = 1:length(lst)
    if contains(lst(j).name, 'passive_allData.mat')
        ...
        nametmp = namesplit(5);
        save([path, 'python\' strjoin(namesplit(2:4), '_') '\' nametmp{1} '_units.mat'], 'units', 'imec')
...
%         end
        ...
        save([path 'python\' strjoin(namesplit(2:4), '_') '\' nametmp{1} '_behavior.mat'], 'R_hit_tmp', 'R_miss_tmp', 'R_ignore_tmp', 'L_hit_tmp', 'L_miss_tmp', 'L_ignore_tmp', 'LickEarly_tmp', 'i_good_trials', 'StimDur_tmp', 'StimLevel')
